...
class FXBook:
    ...
    therefore amount / rate. Foreign-currency trading is fail-closed: a missing,
    malformed, stale, or implausibly future-dated FX timestamp cannot be used.
    """
    ...
    def load(cls, path: str = "data/fx_rates.json") -> "FXBook":
        ...
        rates = {
            str(k).upper(): float(v)
            for k, v in raw.get("foreign_per_eur", raw.get("rates", {})).items()
        }
        rates["EUR"] = 1.0
        return cls(
            rates,
    ...
    def age_hours(self) -> float | None:
        ...
        try:
            ...
            # A materially future-dated book is malformed; do not turn it into age 0.
            if delta < -6.0:
                return None
...
def refresh_ecb(
    path: str = "data/fx_rates.json",
    timeout: float = 20.0,
    friction_pct: float = 0.004,
) -> dict:
    req = urllib.request.Request(
        ECB_DAILY_XML,
        headers={"User-Agent": "ROMAN-paper-research/0.2"},
    )
    ...
    # ECB reference rates are published on a European business-day clock. Use a
    # real timezone rather than hard-coding +02:00, which is wrong in winter.
    ref_date = date.fromisoformat(asof)
    ref_dt = datetime.combine(
        ref_date,
        time(16, 0),
        tzinfo=ZoneInfo("Europe/Frankfurt"),
    )
    payload = {
        "base": "EUR",
        "asof": ref_dt.isoformat(),
        "source": "ECB euro foreign exchange reference rates",
        "foreign_per_eur": rates,
        "friction_pct": float(friction_pct),
        "max_age_hours": 96.0,
        "warning": (
            "reference mid only; ROMAN adds FX friction and does not treat this "
            "as an executable transaction quote"
        ),
    }
    p = Path(path)
