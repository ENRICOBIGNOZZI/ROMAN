...
class ShadowLedger:
    ...
    Marks are *not* realized P&L.  A separate executable_value is stored when a
    fresh bid/locked route is observed.  This distinction is deliberate: a nice
    comparable ask must never be reported as a sale.
    """
    ...
    def cash(self) -> float:
        ...
        return self.capital + float(realized or 0.0) - self.deployed_cost()
    ...
    def mark(self, candidates: list[dict]) -> None:
        ...
        for p in self.open_positions():
            ...
            if c is None:
                # No fresh evidence: preserve entry cost as neutral mark rather
                # than manufacturing a price from stale data.
                ...
                if c.get("locked_net_roi") is not None and bool(c.get("locked")):
                    # Express the current fresh locked ROI on the historical entry
                    # cost.  Still labelled paper-executable, never realized.
                    executable_value = float(p["entry_cost"]) * (1.0 + float(c.get("locked_net_roi") or 0.0))
                meta = {
    ...
    def nav_series(self, limit: int = 1000) -> list[dict]:
        ...
        return [
            {"t": r["observed_at"], "nav": float(r["nav_mark"]), "mark_pnl": float(r["mark_pnl"])}
