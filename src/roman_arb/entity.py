...
# These strings are useful attributes, but they are not product identities.
# Treating e.g. "256GB" or "70-200" as a SKU can merge unrelated products
# across marketplaces and manufacture a completely false arbitrage opportunity.
_GENERIC_CODE_PATTERNS = (
    re.compile(r"^(?:64|128|256|512|1024|2048)(?:gb|tb)$", re.I),
    re.compile(r"^\d{1,3}[-/]\d{1,3}$", re.I),
    re.compile(r"^\d{2,3}mm$", re.I),
    re.compile(r"^f\d+(?:[._]\d+)?$", re.I),
)

# Conservative brand anchors for the initial ROMAN universe. Numeric references
# are only promoted to a cross-market identity when a stable semantic anchor is
# present. It is better to miss a match than to buy the wrong economic object.
_BRAND_ANCHORS = (
    "rolex", "omega", "cartier", "seiko", "tudor", "tag heuer", "breitling",
    "lego", "pokemon", "panini", "topps",
    "nike", "jordan", "adidas", "yeezy", "new balance", "asics",
    "sony", "canon", "nikon", "fujifilm", "leica", "tamron", "sigma",
    "apple", "iphone", "ipad", "samsung", "google", "pixel",
    "nvidia", "geforce", "amd", "radeon",
    "nintendo", "playstation", "xbox", "microsoft",
    "fender", "gibson", "boss", "roland", "korg", "yamaha", "moog",
)
...
def _is_generic_code(code: str) -> bool:
    c = (code or "").lower().strip("-_/. ")
    return any(p.fullmatch(c) for p in _GENERIC_CODE_PATTERNS)


def _brand_anchor(title: str) -> str:
    low = re.sub(r"\s+", " ", (title or "").lower())
    for brand in _BRAND_ANCHORS:
...
def structured_codes(title: str) -> tuple[str, ...]:
    ...
    for raw in _CODE_RE.findall(title or ""):
        t = raw.lower().strip("-_/. ")
        if len(t) < 4 or _is_generic_code(t):
            continue
...
def structured_attributes(title: str, condition: str = "") -> dict[str, str]:
    ...
    if m:
        attrs["grade"] = f"{m.group(1).lower()}:{m.group(2)}"
    m = _STORAGE_RE.search(text)
    if m:
        attrs["storage"] = f"{m.group(1)}{m.group(2).lower()}"
    ...
    if m:
        ...
        if m2:
            attrs["size"] = m2.group(1) + (("." + m2.group(2)) if m2.group(2) else "")
...
def entity_key(row: RawListing | dict) -> str:
    ...
    if codes:
        ...
        suffix = "|".join(
            ...
            if k in {"grade", "size", "storage"}
        )
        anchor = _brand_anchor(title)
        has_alpha = any(c.isalpha() for c in code)
        has_digit = any(c.isdigit() for c in code)

        # Pure numeric references are dangerous without a brand/manufacturer
        # anchor; alphanumeric IDs can stand alone only when sufficiently rich.
        if anchor:
            return "id:" + anchor + ":" + code + ("|" + suffix if suffix else "")
        if has_alpha and has_digit and len(code) >= 6:
            return "id:" + code + ("|" + suffix if suffix else "")
        # Otherwise fall through to a title fingerprint instead of claiming an ID.
...
def match_confidence(a: RawListing | dict, b: RawListing | dict) -> float:
    ...
    if ca and cb:
        if ca & cb:
            # A shared generic-looking code is not enough; structured_codes has
            # already removed storage/focal-length/aperture pseudo-identifiers.
            ba, bb = _brand_anchor(ta), _brand_anchor(tb)
            if ba and bb and ba != bb:
                return 0.01
            base = 0.985 if (not ba or not bb or ba == bb) else 0.80
            shared_attrs = sum(
                attrs_a.get(k) == attrs_b.get(k)
                for k in attrs_a.keys() & attrs_b.keys()
            )
